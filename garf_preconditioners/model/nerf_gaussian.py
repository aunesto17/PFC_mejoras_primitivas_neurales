"""
MODELO NERF-GAUSSIANO (GARF) - IMPLEMENTACION PRINCIPAL DE PRECONDICIONADORES
================================================================================
Este archivo contiene el motor principal de entrenamiento y evaluacion para
NeRF con activaciones Gaussianas, usado en los experimentos de Preconditioners.

ARQUITECTURA DEL MODELO (clase NeRF_Gaussian en Graph):
- MLP de 6 capas, 256 neuronas por capa, activacion Gaussiana (sigma=0.1)
  definida como: exp(-0.5 * x^2 / sigma^2)
- Sin codificacion posicional (a diferencia del NeRF original)
- Skip connection en la capa 3 (concatena entrada a la activacion de capa 3)
- Dos ramas de salida: densidad (softplus) y color RGB (condicionado por vista)

CONFIGURACION DE OPTIMIZADORES (metodo setup_optimizer):
- Adam:       LR=1e-4 constante, schedule exponencial (lr_end=1e-4, sin decaimiento)
- ESGD_Max:   LR=1.0 con warmup (lr_warmup=0.99), decaimiento exponencial a 0.01,
              actualizacion del precondicionador D cada update=100 iteraciones,
              d_warmup=50 (primeras 50 iteraciones siempre calculan HVP),
              preconditioner_type="equilibrated" (norma de filas de la Hessiana)

ENTRENAMIENTO (metodo train):
- Iteraciones definidas por max_iter (200,000 para LLFF)
- Muestreo aleatorio de rand_rays=512 rayos por iteracion
- Renderizado volumetrico con muestreo de profundidad (128 muestras)
- Evaluacion periodica en conjunto de validacion (PSNR en TensorBoard)
- Checkpoints guardados cada 5000 iteraciones

EVALUACION (metodo evaluate_full):
- Renderizado completo de vistas de prueba (por slices para ahorrar memoria)
- Metricas: PSNR, SSIM (pytorch_ssim), LPIPS (AlexNet)
- Generacion de mapas de error y comparaciones visuales
- Resultados guardados en results.json y quant.txt

DIFERENCIA CON LA IMPLEMENTACION nf-soft-mining:
Esta implementacion (GARF) logra entrenar ESGD_Max correctamente (PSNR ~23 en fern),
mientras que la implementacion en nf-soft-mining/train_gaussian_nerf.py no converge
(PSNR ~12). La diferencia clave parece estar en el manejo del scheduler y el
precondicionador: GARF usa decaimiento exponencial de LR y preconditioner_type="equilibrated".

En nuestro trabajo, este modelo se uso para comparar Adam vs ESGD_Max en la
escena fern del dataset LLFF (200K iteraciones, rand_rays=512).
"""
...
class Model(base.Model):
    ...
    def setup_optimizer(self,opt):
        ...
        if opt.optim.algo in ["ESGD", "ESGD_Max"]:
            ...
            esgd_cfg = opt.optim.ESGD
            params = list(self.graph.nerf.parameters())
            self.optim = optimizer(
                params=params,
                lr=esgd_cfg.lr,
                betas=tuple(esgd_cfg.get("betas", [0.9, 0.999])),
                lr_warmup=esgd_cfg.get("lr_warmup", 0.99),
                eps=esgd_cfg.get("damping", 1e-4),
                update_d_every=esgd_cfg.get("update", 100),
                d_warmup=esgd_cfg.get("d_warmup", 50),
                preconditioner_type="equilbrated"
            )
            if opt.nerf.fine_sampling:
                self.optim.add_param_group(dict(params=list(self.graph.nerf_fine.parameters()), lr=esgd_cfg.lr))
        else:
            optimizer = getattr(torch.optim,opt.optim.algo)
            lr = float(opt.optim.lr)
            self.optim = optimizer([dict(params=self.graph.nerf.parameters(),lr=lr)])
            if opt.nerf.fine_sampling:
                self.optim.add_param_group(dict(params=self.graph.nerf_fine.parameters(),lr=lr))
        ## set up scheduler ##
        if opt.optim.sched:
            ...
            if opt.optim.lr_end:
                ...
                lr = float(opt.optim.lr)
                lr_end = float(opt.optim.lr_end)
                max_iter = int(opt.max_iter)
                opt.optim.sched.gamma = (lr_end/lr)**(1./max_iter)
    ...
    def train(self,opt):
        ...
        for self.it in loader:
            ...
            if opt.optim.sched: 
                ...
                decay_steps = float(opt.optim.lr_decay) * 1000
                new_lrate = float(opt.optim.lr) * (0.1 ** (self.it/decay_steps))
    ...
    def evaluate_full(self,opt,eps=1e-10):
        ...
        for i,batch in enumerate(loader):
            ...
            # dump rendered views and ground truth
            ...
            # error maps and side-by-side composites
            self._save_error_map(rgb_map.cpu()[0],var.image.cpu()[0],
                                 "{}/error_{}.png".format(test_path,i))
            self._save_comparison(rgb_map.cpu()[0],var.image.cpu()[0],
                                  "{}/comparison_{}.png".format(test_path,i))
        ...
        # dump structured results as JSON
        self._save_results_json(opt,res)

    def _save_error_map(self,rgb_pred,rgb_gt,path):
        import matplotlib.cm as cm
        err = (rgb_pred-rgb_gt).abs()
        err_gray = err.mean(dim=0).numpy()  # [H,W]
        err_gray = np.clip(err_gray/0.1,0,1)
        err_color = (cm.jet(err_gray)[:,:,:3]*255).astype(np.uint8)
        from PIL import Image
        Image.fromarray(err_color).save(path)

    def _save_comparison(self,rgb_pred,rgb_gt,path):
        import matplotlib.cm as cm
        from PIL import Image
        err = (rgb_pred-rgb_gt).abs()
        err_gray = err.mean(dim=0).numpy()
        err_gray = np.clip(err_gray/0.1,0,1)
        err_color = (cm.jet(err_gray)[:,:,:3]*255).astype(np.uint8)
        gt_np = (rgb_gt.permute(1,2,0).numpy()*255).astype(np.uint8)
        pred_np = (rgb_pred.permute(1,2,0).numpy()*255).astype(np.uint8)
        h,w = gt_np.shape[:2]
        gap = 5
        canvas = np.ones((h,w*3+gap*2,3),dtype=np.uint8)*255
        canvas[:,:w] = gt_np
        canvas[:,w+gap:w*2+gap] = pred_np
        canvas[:,w*2+gap*2:] = err_color
        Image.fromarray(canvas).save(path)

    def _save_results_json(self,opt,res):
        avg = edict(psnr=np.mean([r.psnr for r in res]),
                    ssim=np.mean([r.ssim for r in res]),
                    lpips=np.mean([r.lpips for r in res]))
        results = dict(
            model=opt.model,
            optimizer=opt.optim.algo,
            scene=opt.data.scene,
            max_iter=opt.max_iter,
            rand_rays=opt.nerf.rand_rays,
            per_image=[dict(idx=i,psnr=r.psnr,ssim=r.ssim,lpips=r.lpips) for i,r in enumerate(res)],
            average=dict(psnr=avg.psnr,ssim=avg.ssim,lpips=avg.lpips),
        )
        with open("{}/results.json".format(opt.output_path),"w") as f:
