...
class Model(nerf_gaussian.Model):
    ...
    def setup_optimizer(self,opt):
        ...
        # pose optimizer always uses Adam regardless of main NeRF optimizer
        lr_pose = float(opt.optim.lr_pose)
        self.optim_pose = torch.optim.Adam([dict(params=self.graph.se3_refine.parameters(),lr=lr_pose)])
        # set up scheduler
        if opt.optim.sched_pose:
            ...
            if opt.optim.lr_pose_end:
                ...
                lr_pose_end = float(opt.optim.lr_pose_end)
                max_iter = int(opt.max_iter)
                opt.optim.sched_pose.gamma = (lr_pose_end/lr_pose)**(1./max_iter)
