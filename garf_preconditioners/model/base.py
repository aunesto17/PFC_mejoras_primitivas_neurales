"""
MOTOR DE ENTRENAMIENTO BASE PARA GARF (CON SOPORTE ESGD)
==========================================================
Clase base Model que define el ciclo de entrenamiento generico para todos los
modelos de GARF (NeRF-Gaussian, GARF con optimizacion de pose, etc.).

CARACTERISTICAS CLAVE:
- Soporte dual de optimizadores: Adam (estandar) y ESGD/ESGD_Max (precondicionados)
- El metodo train_iteration_esgd() (linea ~101) ejecuta backward con
  create_graph=True cuando el optimizador lo requiere (should_create_graph()).
  Esto permite calcular productos Hessiano-vector (HVP) para el precondicionador.
- Manejo de reference cycles: zero_grad(set_to_none=True) despues de cada paso
  para liberar el grafo computacional y evitar fugas de memoria con create_graph.
- Checkpointing automatico (save/restore) con metadatos de configuracion.
- Registro en TensorBoard de perdidas, PSNR, y learning rate.

FLUJO DE ENTRENAMIENTO:
1. train(): bucle principal que llama a train_iteration() o train_iteration_esgd()
2. train_iteration_esgd(): backward(create_graph=True) -> calculo de HVP ->
   optimizer.step() -> zero_grad(set_to_none=True)
3. validate(): evaluacion periodica en conjunto de validacion
4. save_checkpoint(): guardado periodico del modelo y optimizador

IMPORTANCIA PARA NUESTRO TRABAJO:
Esta clase implementa el soporte fundamental para entrenar con ESGD/ESGD_Max.
El uso de create_graph=True es el mecanismo que permite a los optimizadores
de segundo orden acceder a la Hessiana a traves de autograd, pero duplica
...
class Model():
    ...
    def train_iteration(self,opt,var,loader):
        # dispatch to ESGD-specific iteration if using ESGD/ESGD_Max
        if opt.optim.algo in ["ESGD", "ESGD_Max"]:
            return self.train_iteration_esgd(opt,var,loader)
    ...
    def train_iteration_esgd(self,opt,var,loader):
        # ESGD/ESGD_Max specific training iteration
        # uses create_graph=True only when needed for Hessian-vector products
        self.timer.it_start = time.time()
        # train iteration
