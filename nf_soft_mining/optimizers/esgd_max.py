...
IMPLEMENTACION ESGD_MAX - VARIANTE CON MAXIMO HISTORICO (nf-soft-mining)
==========================================================================
Variante del optimizador ESGD que usa torch.maximum() en lugar de EMA para
mantener el precondicionador diagonal D. Esto resulta en un precondicionador
mas agresivo que captura la maxima magnitud historica del HVP en cada parametro.

DIFERENCIA CON ESGD ESTANDAR:
- En lugar de exp_avg_d = beta2 * exp_avg_d + (1-beta2) * hvp^2 (EMA),
  usa exp_avg_d = max(exp_avg_d, hvp^2), manteniendo el valor maximo historico.
- Esto hace al precondicionador mas estable pero potencialmente mas conservador
  (nunca decrece, solo aumenta cuando encuentra curvaturas mayores).

NOTA: En el paper de Preconditioners, ESGD_Max mostro mejor rendimiento que
ESGD estandar en tareas de NeRF. Sin embargo, en la implementacion nf-soft-mining
esta version NO converge (PSNR ~12), posiblemente por la combinacion de
hiperparametros (scheduler, LR, batch size) diferente de la implementacion GARF.
"""
...
class ESGD_Max(torch.optim.Optimizer):
    """ESGD-M (ESGD from "Equilibrated adaptive learning rates for non-convex optimization"
    with Nesterov momentum.
