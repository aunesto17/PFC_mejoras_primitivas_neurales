...
IMPLEMENTACION ESGD - OPTIMIZADOR PRECONDICIONADO (nf-soft-mining)
====================================================================
Implementacion del optimizador Equilibrated Stochastic Gradient Descent con
momento de Nesterov. Utiliza el estimador de traza de Hutchinson con vectores
Rademacher para aproximar la diagonal de la Hessiana.

ALGORITMO:
1. should_create_graph(): determina cuando se necesita calcular HVP (durante
   warm-up y cada update_d_every pasos).
2. En cada paso donde se requiere, se calcula el producto Hessiano-vector (HVP)
   usando vectores aleatorios de Rademacher como estimador de Hutchinson.
3. La diagonal del precondicionador D se actualiza via EMA del HVP al cuadrado.
4. El update final aplica momento de Nesterov y escala la gradiente por D^{-1/2}.

NOTA: Esta implementacion usa la clase Optimizer local (no torch.optim.Optimizer).
Es parte del codebase nf-soft-mining y se usa en train_gaussian_nerf.py.
"""
