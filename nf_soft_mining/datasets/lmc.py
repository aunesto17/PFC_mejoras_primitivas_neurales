"""
IMPLEMENTACION DEL ALGORITMO SOFT MINING VIA LANGEVIN MONTE CARLO (LMC)
========================================================================
Este modulo es el nucleo de la tecnica Soft Mining (Kheradmand et al., CVPR 2024).
Reemplaza el muestreo uniforme de rayos/pixeles en el entrenamiento de NeRF por un
muestreo de importancia guiado por el error de reconstruccion.

FUNCIONAMIENTO:
- Mantiene un conjunto de particulas (coordenadas 2D en el espacio de imagen) que
  se actualizan iterativamente mediante dinamica de Langevin: la gradiente del error
  de la red neuronal respecto a las coordenadas 2D guia las particulas hacia regiones
  con alto error (regiones "duras" de aprender).
- Ruido Gaussiano inyectado en cada paso (parametro b) permite exploracion estocastica
  y evita que las particulas queden atrapadas en minimos locales.
- Las particulas que producen bajo error o salen de los limites de la imagen son
  reinicializadas usando una distribucion basada en bordes de Sobel (image_edges).
- Parametros clave: minpct (fraccion minima de reinicializacion uniforme, 10%),
  lossminpc (fraccion reinicializada por bajo error, 10%), a (tamano de paso de
  gradiente, 2e1 para LLFF), b (tamano de paso de ruido, 2e-2 para LLFF).

En nuestro trabajo, LMC se aplica sobre la arquitectura Instant-NGP y se compara
contra el muestreo uniforme tradicional en las 8 escenas del dataset LLFF.
"""
